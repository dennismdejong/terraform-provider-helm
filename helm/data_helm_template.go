...
func (d *HelmTemplate) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if !state.RepositoryPasswordWO.IsNull() && state.RepositoryPasswordWO.ValueString() != "" {
...
func chartPathOptionsModel(model *HelmTemplateModel, meta *Meta, cpo *action.ChartPathOptions) (*action.ChartPathOptions, string, diag.Diagnostics) {
	...
	repoPassword := model.RepositoryPassword.ValueString()
	if !model.RepositoryPasswordWO.IsNull() && model.RepositoryPasswordWO.ValueString() != "" {
		repoPassword = model.RepositoryPasswordWO.ValueString()
	}
	cpo.Password = repoPassword
